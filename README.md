...
视觉模板配置放在程序目录的 `data\vision\profile.json`，由 `layout`（商店、手牌、场面和卡槽的归一化坐标）、`anchors`（`shop`、`hand`、`board` 锚点图片）、`scenes`（场景模板图片）和 `digits`（金币/等级模板图片）组成。模板文件路径必须相对于 `profile.json`，程序会拒绝越界路径、损坏图片和缺失必需锚点。没有该配置时，“开始运行”会保持暂停，这是预期的安全行为。
