...
- 商店卡牌使用专用缩略图特征：从官网完整卡图提取插画区域，再与游戏商店牌面匹配；金色随从可匹配到普通卡 ID，酒馆法术和低置信度结果保持 UNKNOWN。
...
同步前暂停运行并保留当前规则，完整下载和图片校验后替换卡库；接口出错、分页重复或缺失时保留旧库。版本标识为内容指纹，表示数据变化，不代表国服客户端补丁号。卡图和普通卡 ORB 特征保存在本地；程序启动时会从当前版本卡图生成商店缩略图特征，因此卡库更新后不需要重新标定识别代码。
